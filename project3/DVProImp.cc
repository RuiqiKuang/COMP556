...
//            }
        ...
        unsigned short totalCost = DVentry.second + get<0>(neighbor_table[port_table[port]]);
...
void DVProImp ::update(unsigned short id, unsigned short RTT, unsigned short oldID) {
    if (oldID != id) {
        unsigned short nextHop = routing_table[oldID].second;
        if (nextHop == oldID) {
            routing_table.erase(oldID);
            DV_time.erase(oldID);
        }
    }
